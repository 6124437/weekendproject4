...
export class AppService {
  ...
  constructor() {
    this.provider = new ethers.providers.AlchemyProvider(
      "goerli",
      "ey4KLDzafnyJYbGLE_UW2qfMrbXLRZ6z"
    );
    this.tokenContract = new ethers.Contract(TOKEN_CONTRACT_ADDRESS, tokenJson.abi, this.provider);
    this.ballotContract = new ethers.Contract(BALLOT_CONTRACT_ADDRESS, ballotJson.abi, this.provider);
  }
  ...
  async delegateTokens()  {
    const wallet = new ethers.Wallet(privateKey).connect(this.provider);
    const tx = await this.tokenContract.connect(wallet).delegate(wallet.address);
    const txReceipt = await tx.wait();
    return txReceipt.status == 1 ? "Completed" : "Reverted";
  }
